...
- Any message containing: **dokuwiki**, **wiki page**, **wikipage**, **namespace**, **append to wiki**, **update wiki**, **draft wiki**, **save to wiki**, **wiki entry**
...
When the user says **"update dokuwiki"**, **"manage dokuwiki"**, or similar + a URL/link:

- **This skill (`dokuwiki-manager`) takes priority** over other skills (including `medical-dokuwiki-writer`).
- You **must** activate this skill and follow the full two-stage interactive workflow.
- Do **not** let other skills take over or auto-write to the wiki.
- If another skill is currently active, clearly state: "Switching to DokuWiki Manager for structured update..." and proceed with Stage 1.
...
- **Smart Merge Rules**: Preserve 100% of the original page structure, images, tables, and existing sections. Only add new content or enhance existing sections. Never remove or overwrite original content unless the user explicitly instructs you to.
...
- Fetch the original page content via MCP if performing an update.
- Translate and compile the combined/new content into pristine, native **DokuWiki Syntax** (e.g., `=` for headers, `*` for lists, `**` for bold, etc.).
- Wrap the **COMPLETE** generated syntax inside a standard Markdown code block tagged with `dokuwiki`. **The user must be able to review the full final page state before anything is written.**
...
```
### Stage 2: Full Layout Preview (After path/method is decided or for Scenarios B & C)
```markdown
### 🔍 Processing Summary
...
### 📄 DokuWiki Page Preview
```dokuwiki
[Insert FULL DokuWiki syntax here - must show the ENTIRE page text if updated]
...
Ensure you map HTML/Markdown elements strictly to DokuWiki syntax guidelines:
- Headers: `====== Document Title ======` (H1) down to `== Subheading ==` (H5)
- Formatting: `**bold**`, `//italic//`, `__underline__`, `''monospace''`
- Lists: Unordered start with two spaces and a `*`, ordered start with two spaces and a `-`
